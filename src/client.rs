    ...
    async fn authenticate(
        ...
        match auth {
            ...
            AuthMethod::PrivateKey { key_data, key_pass } => {
                ...
                let cprivk = PrivateKeyWithHashAlg::new(Arc::new(cprivk), None);
                let is_authentificated = handle
                    .authenticate_publickey(username, cprivk)
                    .await?;
                if !is_authentificated.success() {
            ...
            AuthMethod::PublicKeyFile { key_file_path } => {
                let cpubk =
                    russh::keys::load_public_key(key_file_path).map_err(crate::Error::KeyInvalid)?;
                ...
                let result = handle.authenticate_publickey_with(username, cpubk, None, &mut agent).await;
                if !result.map_err(crate::Error::AgentAuthError)?.success() {
                    return Err(crate::Error::KeyAuthFailed);
                }
            }
            AuthMethod::KeyboardInteractive(mut kbd) => {
                ...
                loop {
                    let prompts = match res {
                        ...
                        KeyboardInteractiveAuthResponse::Failure { remaining_methods: _ } => {
